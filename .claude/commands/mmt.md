...
**Status lines & foreground:** as plain text (never `echo`), write `▶ <who> — <does>` before each subagent and `✔ <who> — <one-line verdict/summary>` after. Run each member subagent **synchronously in the foreground** (wait for it before the next step) — never in the background. That, plus the subagent blocks, is the whole observability.
