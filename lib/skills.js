...
// Native Claude precedence for skills: user (~/.claude) beats project (.claude).
// The legacy ~/.my-mini-team/skills dir is kept last as a fallback so skills
// authored before the native move still resolve.
function sources() {
  return [
...
// Native skill location: ~/.claude/skills (global/personal) or ./.claude/skills
// (project). This is where plain Claude Code discovers skills too.
function skillDir(local) {
  return local
    ? path.join(process.cwd(), '.claude', 'skills')
    : path.join(os.homedir(), '.claude', 'skills');
}

// Write a skill to its native location. opts.local → project scope.
function writeSkill(name, content, opts = {}) {
  const file = path.join(skillDir(!!opts.local), name, 'SKILL.md');
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.writeFileSync(file, content);
  return file;
}

// Delete a skill across every source: dir-form (<name>/SKILL.md) removes the
// whole <name>/ directory; flat-form (<name>.md) unlinks the file. Returns the
// removed paths (directories for dir-form, files for flat-form).
function deleteSkill(name) {
  const removed = [];
  for (const { dir } of sources()) {
    const dirForm = path.join(dir, name);
    if (fs.existsSync(path.join(dirForm, 'SKILL.md'))) {
      fs.rmSync(dirForm, { recursive: true, force: true });
      removed.push(dirForm);
      continue;
    }
    const flat = path.join(dir, name + '.md');
    if (fs.existsSync(flat)) { fs.unlinkSync(flat); removed.push(flat); }
  }
  return removed;
}

module.exports = { resolve, list, mmtSkillPath, writeMmtSkill, skillDir, writeSkill, deleteSkill, descOf, sources };
