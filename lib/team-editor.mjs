...
export function AgentEditor({ agent, skillNames, onSave, onCancel }) {
  ...
  useInput((input, key) => {
    dbg('AGENT ' + JSON.stringify({ input, up: key.upArrow, down: key.downArrow, ret: key.return }));
    if (key.escape) return onCancel();
    if (key.return) return save();
    ...
    if (idx === 1) { // model row: space / ←→ cycles
      const j = MODELS.indexOf(model);
      if (input === ' ' || key.rightArrow) return setModel(MODELS[(j + 1) % MODELS.length]);
      if (key.leftArrow) return setModel(MODELS[(j - 1 + MODELS.length) % MODELS.length]);
      return;
    }
