...
// Env-gated keypress logging (MMT_EDITOR_DEBUG=1 → /tmp/mmt-editor.log) to
// diagnose a frozen-cursor report: entries = useInput fires; empty = keys never
// arrive (a terminal/raw-mode problem, not this code).
const dbg = (m) => { if (process.env.MMT_EDITOR_DEBUG) { try { fs.appendFileSync('/tmp/mmt-editor.log', m + '\n'); } catch (e) {} } };
...
function ListView({ steps, setSteps, name, onEdit, onExit }) {
  ...
  useInput((input, key) => {
    dbg('LIST ' + JSON.stringify({ input, up: key.upArrow, down: key.downArrow }));
...
export function AgentEditor({ agent, skillNames, onSave, onCancel }) {
  const [field, setField] = useState('desc'); // desc | model | skills
  ...
  const rows = forms.filterList(allSkills, filter).map((name) => ({ name, checked: desired.includes(name) }));
  const save = () => onSave({ description, model, skills: desired });

  useInput((input, key) => {
    dbg('AGENT ' + JSON.stringify({ input, tab: key.tab, up: key.upArrow, down: key.downArrow, ret: key.return, esc: key.escape }));
    if (key.escape) return onCancel();
    if (key.return) return save();
    if (key.tab) return setField((f) => (f === 'desc' ? 'model' : f === 'model' ? 'skills' : 'desc'));
    if (field === 'desc') return setDescription((b) => forms.applyChar(b, input, key));
    if (field === 'model') {
      if (key.upArrow || key.downArrow) {
        const i = MODELS.indexOf(model);
        const ni = key.downArrow ? (i + 1) % MODELS.length : (i - 1 + MODELS.length) % MODELS.length;
        return setModel(MODELS[ni]);
      }
      return;
    }
    if (field === 'skills') {
      ...
      if (key.downArrow) return setSkillIdx((i) => Math.min(rows.length - 1, i + 1));
      if (input === ' ') { const r = rows[skillIdx]; if (r) setDesired((d) => forms.toggleSkill(d, r.name)); return; }
      if (key.backspace || key.delete) return setFilter((s) => s.slice(0, -1));
      if (input && input.length === 1 && input >= '!' && !key.ctrl) return setFilter((s) => s + input);
      return;
    }
  });
  ...
  return h(Box, { flexDirection: 'column' },
    ...
    h(Box, { flexDirection: 'column', marginTop: 1 },
      h(Text, null, '  ', label('desc', 'description  '), field === 'desc' ? h(Text, { color: 'cyan' }, (description || '') + '▏') : h(Text, null, description || ' ')),
      h(Text, null, '  ', label('model', 'model        '), (model || 'inherit') + (field === 'model' ? '   (↑↓ change)' : '')),
      h(Box, { flexDirection: 'column' },
        h(Text, null, '  ', label('skills', 'skills'), field === 'skills' && filter ? h(Text, { color: 'gray' }, '   filter: ' + filter) : ''),
        ...rows.map((r, k) => h(Text, { key: r.name, color: field === 'skills' && k === skillIdx ? 'cyan' : undefined },
          '      ' + (field === 'skills' && k === skillIdx ? '❯ ' : '  ') + '[' + (r.checked ? 'x' : ' ') + '] ' + r.name)))),
    h(Text, { color: 'gray' }, '\n  tab next field · ↑↓ move/change · space toggle · ⏎ save · esc cancel'));
}
