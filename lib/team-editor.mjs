...
function StepEditor({ step, agentList, skillNames, onSave, onCancel }) {
  ...
  const [agentIdx, setAgentIdx] = useState(Math.max(0, agentNames.indexOf(startName)));
  const [does, setDoes] = useState(step.does || '');
  const [desired, setDesired] = useState(startDesired);
  const [skillIdx, setSkillIdx] = useState(0);

  const filtered = forms.filterList(agentNames, filter);
  ...
  const save = () => onSave({
    uses: agentName, member: agentName, does,
