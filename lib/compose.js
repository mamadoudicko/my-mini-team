...
const AGENT_SCHEMA = `You write a native Claude Code subagent file for the "mmt" tool. A subagent is a delegate Claude can hand a focused job to: a system-prompt persona with a clear role. Output ONLY the file content — no prose, no explanations, no code fences.

Exact format:
---
name: <kebab-name>
description: <one sentence saying when to delegate to this agent — this is what Claude routes to the agent by>
model: sonnet
skills: []
---
...
<a concise system prompt: who this agent is, what it is responsible for, how it should approach the work, and what "done" looks like>`;

// Compose a native subagent .md from a plain-language role description.
async function composeAgent(name, role) {
...
module.exports = { compose, edit, composeSkill, composeAgent, runClaude };
