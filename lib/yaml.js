...
function stripComment(line) {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if (quote) {
      ...
      break; // only a comment at line start or after whitespace (keep e.g. issue#123)
    } else {
      out += c;
    }
  }
...
function unquote(v) {
  if (v.startsWith('"') && v.endsWith('"')) {
    // Un-escape \" and \\ inside a double-quoted scalar (symmetric with the
    // writer in serialize.js), in a single left-to-right pass.
